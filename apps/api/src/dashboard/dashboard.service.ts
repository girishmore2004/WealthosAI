...
export class DashboardService {
  ...
  async getSummary(userId: string): Promise<DashboardSummaryDTO> {
    ...
    const [
      ...
      this.propertyService.totalCurrentValue(userId),
      this.prisma.client.budget.findMany({ where: { userId } }),
      // Used by both the goal-based emergency-fund fix (#2) and the uncommitted-cash
      // metric (#18) below. `investments: true` lets uncommittedCash tell apart goals
      // that are just a cash target (currentAmount) from goals already backed by a
      // linked Investment (whose value is already counted in investmentsValue, so
      // subtracting its currentAmount from cash again would double-count it).
    ...
    // #2 fix: emergency fund is no longer coupled to an Expense row whose category
    // happens to be named exactly "Emergency Fund". GoalType.EMERGENCY_FUND already
    // exists in the schema — prefer summing currentAmount across any goal(s) of that
    // type (a user's actual, dedicated liquid-reserve target), and only fall back to
    // the old category-name match for accounts that have logged an "Emergency Fund"
    // expense but never created the matching goal, so nothing that worked before
    // silently regresses to 0.
    const emergencyFundGoals = goals.filter((g) => g.type === "EMERGENCY_FUND");
    const emergencyFundCategory = monthExpenses.find((e) => e.category.name === "Emergency Fund");

    let emergencyFundAmount = 0;
    let emergencyFundBasis: "GOAL" | "CATEGORY_LEGACY" | "NONE" = "NONE";

    if (emergencyFundGoals.length > 0) {
      emergencyFundAmount = emergencyFundGoals.reduce((sum, g) => sum + Number(g.currentAmount), 0);
      emergencyFundBasis = "GOAL";
    } else if (emergencyFundCategory) {
      emergencyFundAmount = Number(emergencyFundCategory.amount);
      emergencyFundBasis = "CATEGORY_LEGACY";
    }

    const emergencyFundMonths =
      monthlyExpenseTotal > 0 && emergencyFundAmount > 0
        ? emergencyFundAmount / (monthlyExpenseTotal / 12)
        : 0;
    ...
    // investmentsValue, so excluding those here avoids double-subtracting it). This is
    // explicitly an approximation — goal.currentAmount is a user-entered figure, not a
    // ledger transfer out of the cash cashBalance is derived from — so it's surfaced as
    // its own clearly-labeled field rather than folded into cashBalance itself.
    const cashEarmarkedForGoals = goals
      .filter((g) => g.investments.length === 0)
      .reduce((sum, g) => sum + Number(g.currentAmount), 0);
    const uncommittedCash = cashBalance - cashEarmarkedForGoals;
    ...
    return {
      ...
      propertyValue: propertyValue.toFixed(2),
      unreadAlertCount: alerts.filter((a) => !a.isRead).length,
      uncommittedCash: uncommittedCash.toFixed(2),
      emergencyFundBasis,
