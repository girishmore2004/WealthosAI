...
export class DashboardService {
  ...
  async getSummary(userId: string): Promise<DashboardSummaryDTO> {
    ...
    // Passing the already-fetched `goals`/`monthExpenses` avoids re-issuing the
    // identical queries a second time within this same request.
    const emergencyFundGoals = goals.filter((g) => g.type === "EMERGENCY_FUND");
    const emergencyFundStatus = await this.financialFactsService.getEmergencyFundStatus(
      userId,
      monthlyExpenseTotal,
