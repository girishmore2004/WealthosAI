...
export default function CoachPage() {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl text-ink sm:text-3xl">AI Coach</h1>
          <p className="text-sm text-ink-soft">
            {mode === "v1"
              ? "Grounded answers from your own data — not a general-purpose chatbot. Every answer is deterministic code, no external model involved."
              : "An explanation layer over the same grounded router — it plans, gathers facts, composes an answer, and verifies every figure before showing it to you."}
        ...
        <div className="flex shrink-0 rounded-md border border-line bg-surface p-1 shadow-card">
          <button
            onClick={() => setMode("v1")}
            className={`rounded-md px-3 py-1.5 text-xs transition-colors ${mode === "v1" ? "bg-marigold-50 font-medium text-marigold-600" : "text-ink-soft hover:text-ink"}`}
          >
            Deterministic
          </button>
          <button
            onClick={() => setMode("v2")}
            className={`rounded-md px-3 py-1.5 text-xs transition-colors ${mode === "v2" ? "bg-marigold-50 font-medium text-marigold-600" : "text-ink-soft hover:text-ink"}`}
          >
            Advanced
          </button>
        </div>
...
function DeterministicCoach() {
  ...
  return (
    <div className="space-y-8">
      <Card>
        <p className="stat-label mb-2.5">Try asking</p>
        <div className="flex flex-wrap gap-2">
          {SUGGESTED_QUESTIONS.map((q) => (
            <button
              key={q}
              onClick={() => ask(q)}
              disabled={asking}
              className="rounded-full border border-line bg-surface-muted px-3 py-1.5 text-xs text-ink-soft transition-colors hover:border-marigold-500 hover:text-marigold-600"
            >
              {q}
            </button>
          ))}
      ...
      <div className="space-y-3">
        ...
        {history.map((h) => (
          <Card key={h.id} hover className={h.wasRefused ? "border-marigold-400" : undefined}>
            <p className="text-sm font-medium text-ink">{h.question}</p>
            <p className="mt-2 text-sm text-ink-soft">{h.answer}</p>
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              {h.wasRefused ? (
                <span className="badge badge-warning">Unsupported question</span>
              ) : (
                h.dataSources.map((s) => (
  ...
  return (
    <div className="space-y-8">
        ...
        {INTENT_FILTERS.map((f) => (
          <button
            ...
            className={`rounded-full border px-3 py-1 text-xs transition-colors ${
              intentFilter === f.value ? "border-marigold-500 bg-marigold-50 font-medium text-marigold-600" : "border-line text-ink-soft hover:text-ink"
            }`}
          >
            {f.label}
          </button>
      ...
      <div className="space-y-3">
        ...
        {filtered.map((h) => (
          <Card key={h.id} hover className={!h.verificationPassed ? "border-marigold-400" : undefined}>
            <div className="flex items-start justify-between gap-3">
              <p className="text-sm font-medium text-ink">{h.question}</p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-ink-faint">
              <ConfidenceBadge value={Number(h.confidence)} />
              {!h.verificationPassed && (
                <span className="badge badge-warning">Composed answer didn&apos;t verify — showing raw facts</span>
              )}
              <button onClick={() => toggleExpanded(h.id)} className="underline hover:text-marigold-600">
                {expandedIds.has(h.id) ? "Hide evidence & plan" : "Show evidence & plan"}
              </button>
            ...
            {expandedIds.has(h.id) && (
              <div className="mt-3 space-y-3 border-t border-line pt-3">
                ...
                <div>
                  ...
                  <pre className="mt-1 overflow-x-auto rounded-md bg-surface-muted p-3 text-[11px] text-ink-soft">
                    {JSON.stringify(h.facts, null, 2)}
                  </pre>
                </div>
                {h.citedSources.length > 0 && (
                  <div>
                    <p className="stat-label">Linked sources</p>
