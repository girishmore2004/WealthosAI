...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {error && <div className="panel border-loss/30 bg-loss/5 p-4 text-sm text-loss">{error}</div>}
      ...
      <Card title="Notifications">
        <div className="flex items-center justify-between py-2.5">
          ...
          <input
            ...
            checked={settings.notifyEmail}
            onChange={(e) => save({ notifyEmail: e.target.checked })}
            className="h-4 w-4 accent-marigold-500 cursor-pointer"
          />
        </div>
        <div className="ledger-rule flex items-center justify-between py-3">
          <p className="text-sm text-ink">Digest frequency</p>
          <select
            value={settings.digestFrequency}
            onChange={(e) => save({ digestFrequency: e.target.value as DigestFrequency })}
            className="rounded-md border border-line bg-surface px-3 py-2 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30"
          >
            {DIGESTS.map((d) => (
              <option key={d} value={d}>
                {d.charAt(0) + d.slice(1).toLowerCase()}
      ...
      <Card title="Privacy & data">
        ...
        <div className="ledger-rule flex items-center justify-between py-3">
          ...
          {!confirmDelete ? (
            <Button variant="secondary" onClick={() => setConfirmDelete(true)} className="border-loss/40 text-loss hover:border-loss hover:bg-loss/5">
              Delete
            </Button>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <Button onClick={onDelete} disabled={deleting} className="bg-loss shadow-none hover:bg-loss/90">
                {deleting ? "Deleting…" : "Confirm delete"}
              </Button>
            </div>
          )}
