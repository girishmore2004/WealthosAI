...
export default function HouseholdPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">{summary.householdName}</h1>
        <p className="text-sm text-ink-soft">
          {summary.memberCount} member{summary.memberCount === 1 ? "" : "s"} · viewing as{" "}
          <span className="font-medium">{summary.viewerRole === "OWNER" ? "Owner" : "Member"}</span>
        </p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <p className="stat-label">Household net worth</p>
          <p className="money mt-1.5 text-2xl font-semibold text-ink">{formatINR(summary.totalNetWorth)}</p>
        </Card>
        <Card>
          <p className="stat-label">Combined monthly income</p>
          <p className="money mt-1.5 text-2xl font-semibold text-gain">{formatINR(summary.totalMonthlyIncome)}</p>
        </Card>
        <Card>
          <p className="stat-label">Combined monthly expenses</p>
          <p className="money mt-1.5 text-2xl font-semibold text-loss">{formatINR(summary.totalMonthlyExpenses)}</p>
        </Card>
        <Card>
          <p className="stat-label">Open alerts</p>
          <p className="money mt-1.5 text-2xl font-semibold text-ink">{summary.totalUnreadAlerts}</p>
        </Card>
      </div>
            ...
            <table className="data-table min-w-[560px]">
              <thead>
                <tr>
                  ...
                  <th className="text-right">Income/mo</th>
                  <th className="text-right">Expenses/mo</th>
              ...
              <tbody>
                {summary.members.map((m) => (
                  <tr key={m.userId}>
                    ...
                    <td className="money text-right">{formatINR(m.netWorth)}</td>
                    <td className="money text-right text-gain">{formatINR(m.monthlyIncome)}</td>
                    <td className="money text-right text-loss">{formatINR(m.monthlyExpenses)}</td>
                    <td className="text-right">{m.goalCount}</td>
                    <td className="text-right">{m.unreadAlertCount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
