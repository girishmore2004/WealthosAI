...
export default function TaxPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">Tax planning · FY {financialYear}</h1>
        <p className="text-sm text-ink-soft">
          An educational estimate to compare regimes and track deductions — not a substitute for a CA or the
          official IT department calculator.
        </p>
      ...
      {estimate && (
        <Card>
          <div className="grid gap-6 sm:grid-cols-2">
            <div className={`rounded-md border p-4 ${estimate.recommendedRegime === "OLD" ? "border-marigold-500" : "border-line"}`}>
              <p className="stat-label">Old regime</p>
              <p className="mt-2 money text-xl text-ink">{formatINR(estimate.oldRegime.taxPayable)}</p>
              <p className="mt-1 text-xs text-ink-faint">
                Taxable income: <span className="money">{formatINR(estimate.oldRegime.taxableIncome)}</span>
              </p>
            ...
            <table className="data-table min-w-[400px]">
              <thead>
                <tr>
                  ...
                  <th className="text-right">Room left</th>
                </tr>
              </thead>
              <tbody>
                {estimate.deductionsBySection.map((row) => (
                  <tr key={row.section}>
                    <td className="text-ink">{row.section.replace(/_/g, " ")}</td>
                    <td className="money text-right">{formatINR(row.used)}</td>
                    <td className="money text-right">{row.limit === "No fixed cap" ? row.limit : formatINR(row.limit)}</td>
                    <td className="money text-right">{formatINR(row.remainingRoom)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
