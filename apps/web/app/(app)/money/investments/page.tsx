...
export default function InvestmentsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card title="Add holding">
        <form onSubmit={onSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <select value={type} onChange={(e) => setType(e.target.value as InvestmentType)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
            {TYPES.map((t) => (
              <option key={t} value={t}>
                {t.replace("_", " ").toLowerCase()}
              </option>
          ...
          <Input type="number" min="0" step="0.01" placeholder="Current value (₹)" value={currentValue} onChange={(e) => setCurrentValue(e.target.value)} required className="money" />
          <Input type="number" min="0" step="0.01" placeholder="Cost basis / invested (₹)" value={costBasis} onChange={(e) => setCostBasis(e.target.value)} required className="money" />
          <select value={riskLevel} onChange={(e) => setRiskLevel(e.target.value as RiskLevel)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
            {RISK_LEVELS.map((r) => (
              <option key={r} value={r}>
                {r.toLowerCase()} risk
              </option>
          ...
          <select value={liquidity} onChange={(e) => setLiquidity(e.target.value as Liquidity)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
            {LIQUIDITY_LEVELS.map((l) => (
              <option key={l} value={l}>
                {l.replace("_", " ").toLowerCase()}
              </option>
