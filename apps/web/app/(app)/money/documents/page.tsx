...
export default function DocumentsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">Documents</h1>
        <p className="text-sm text-ink-soft">
          PAN, Aadhaar, salary slips, policies, statements — stored securely with expiry tracking.
        </p>
      </div>
      ...
      <Card title="Upload a document">
        <form onSubmit={onUpload} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <input
            ...
            onChange={(e) => setFile(e.target.files?.[0] ?? null)}
            accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx"
            className="rounded-md border border-line bg-surface px-3 py-2 text-sm file:mr-2 file:rounded-md file:border-0 file:bg-marigold-50 file:px-2 file:py-1 file:text-marigold-600"
          />
          <select value={category} onChange={(e) => setCategory(e.target.value as DocumentCategory)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
            {CATEGORIES.map((c) => (
              <option key={c} value={c}>
                {c.replace(/_/g, " ")}
              </option>
      ...
      <div className="flex items-center gap-2">
        ...
        <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="rounded-md border border-line bg-surface px-3 py-1.5 text-sm">
          <option value="">All categories</option>
          {CATEGORIES.map((c) => (
            <option key={c} value={c}>
              {c.replace(/_/g, " ")}
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {doc.tags.length > 0 && (
                        <div className="mt-1 flex gap-1">
                          {doc.tags.map((t) => (
                            <span key={t} className="rounded-md bg-paper px-1.5 py-0.5 text-[10px] text-ink-faint">
                              {t}
                            </span>
                          ))}
                        </div>
