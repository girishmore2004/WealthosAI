...
export default function PropertyPage() {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl text-ink sm:text-3xl">Property</h1>
          <p className="text-sm text-ink-soft">Houses, land, and rental units — valuation, equity, and yield.</p>
        </div>
        <Button onClick={() => setShowForm((s) => !s)} variant="secondary">
          {showForm ? "Cancel" : "Add property"}
      ...
      {summary && (
        <div className="grid gap-4 sm:grid-cols-2">
          <Card>
            <p className="stat-label">Total current value</p>
            <p className="money mt-1.5 text-2xl font-semibold text-ink">{formatINR(summary.totalCurrentValue)}</p>
          </Card>
          <Card>
            <p className="stat-label">Total equity</p>
            <p className="money mt-1.5 text-2xl font-semibold text-ink">{formatINR(summary.totalEquity)}</p>
          </Card>
        </div>
      )}
      ...
      {showForm && (
        <Card title="Add a property">
          <form onSubmit={onSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <select value={type} onChange={(e) => setType(e.target.value as PropertyType)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
              {TYPES.map((t) => (
                <option key={t} value={t}>
                  {t.charAt(0) + t.slice(1).toLowerCase()}
                </option>
