...
export default function BusinessPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">Business</h1>
        <p className="text-sm text-ink-soft">Revenue, expenses, profit, and key dates for your business.</p>
      </div>
      ...
      {selectedId && selectedBusiness && summary && (
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Card>
              <p className="stat-label">Revenue ({summary.month})</p>
              <p className="money mt-1.5 text-2xl font-semibold text-gain">{formatINR(summary.revenue)}</p>
            </Card>
            <Card>
              <p className="stat-label">Expenses</p>
              <p className="money mt-1.5 text-2xl font-semibold text-loss">{formatINR(summary.expenses)}</p>
            </Card>
            <Card>
              <p className="stat-label">Profit</p>
              <p className="money mt-1.5 text-2xl font-semibold text-ink">{formatINR(summary.profit)}</p>
            </Card>
          </div>
          ...
          <Card title="Log a transaction">
            <form onSubmit={onAddTransaction} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <select value={txnType} onChange={(e) => setTxnType(e.target.value as BusinessTransactionType)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
                {TXN_TYPES.map((t) => (
                  <option key={t} value={t}>
                    {label(t)}
                  </option>
                  ...
                  editingObligationId === o.id ? (
                    ...
                    <li key={o.id} className="flex items-center justify-between py-1.5 text-sm ledger-rule">
                      <span className="text-ink">
                        ...
                        {o.vendor ? ` · ${o.vendor}` : ""}
                        <span
                          className={`ml-2 rounded-md px-1.5 py-0.5 text-[11px] uppercase tracking-wide ${
                            o.status === "PAID"
                              ? "bg-gain/10 text-gain"
                              : o.status === "OVERDUE"
                                ? "bg-loss/10 text-loss"
