...
export default function LoansPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">Loans &amp; Debt</h1>
        <p className="text-sm text-ink-soft">Every EMI, tracked against your income.</p>
      </div>
      ...
      {summary && (
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div>
              <p className="stat-label">Debt stress score</p>
              <p className={`money text-xl ${summary.debtStressScore > 40 ? "text-loss" : "text-gain"}`}>
                {summary.debtStressScore}%
              </p>
            </div>
      ...
      <Card title="Add loan">
        <form onSubmit={onSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <select value={type} onChange={(e) => setType(e.target.value as LoanType)} className="rounded-md border border-line bg-surface px-3 py-2.5 text-sm text-ink transition-colors focus:border-marigold-500 focus:ring-1 focus:ring-marigold-500/30">
            {TYPES.map((t) => (
              <option key={t} value={t}>
                {t.replace("_", " ").toLowerCase()}
              </option>
