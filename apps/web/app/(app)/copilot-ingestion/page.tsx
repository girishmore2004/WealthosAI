...
export default function CopilotIngestionPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl text-ink sm:text-3xl">Copilot Ingestion</h1>
        <p className="text-sm text-ink-soft">
          Paste a bank or card statement (or its OCR&apos;d text). Nothing is added to your expenses until you
          review and approve each line — this only ever stages suggestions.
        </p>
      ...
      <Card title="Import a statement">
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <textarea
            ...
            placeholder={"15/01/2026, POS AMAZON.IN 4829102, 1249.00 Dr\n16/01/2026, UPI-SWIGGY BANGALORE, 350.50 Dr\n..."}
            rows={8}
            className="w-full rounded-md border border-line bg-surface p-3 font-mono text-xs"
          />
          <Button type="submit" disabled={creating || !rawText.trim() || !sourceLabel.trim()}>
            {creating ? "Processing…" : "Process statement"}
          </Button>
          ...
          <div className="flex flex-wrap gap-2">
            {history.map((b) => (
              <button
                key={b.id}
                onClick={() => loadBatch(b.id)}
                className="rounded-md border border-line px-3 py-1.5 text-xs text-ink-soft hover:border-marigold-500 hover:text-marigold-600"
              >
                {b.sourceLabel} · {b._count.items} lines
              </button>
            ))}
  ...
  if (item.status !== "PENDING") {
    return (
      <div className="flex items-center justify-between border-b border-line py-2.5 text-sm last:border-b-0">
        <span className="text-ink-soft">{item.merchantNormalized}</span>
        <span className={`badge ${item.status === "APPROVED" ? "badge-success" : "badge-info"}`}>{item.status.toLowerCase()}</span>
      </div>
    );
  }
  ...
  return (
    <div className="panel panel-hover p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`stat-value text-xs ${tier}`}>{Math.round(confidence * 100)}% confidence</span>
      </div>

      <p className="mt-2 text-xs text-ink-soft">{item.rationale}</p>

      <div className="mt-2.5 flex flex-wrap gap-1.5">
        {item.isDuplicateCandidate && <span className="badge badge-warning">Possible duplicate</span>}
        {item.isAnomalyCandidate && <span className="badge badge-warning">Unusual amount</span>}
        {item.isRecurringCandidate && <span className="badge badge-info">Matches a subscription</span>}
        {item.missingFields.map((f) => (
